...
const supaQuizP4Q1 = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, {
  auth: {
    persistSession: true,
...
console.log('✅ Supabase initialized for Aralin 4 Quiz 1');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function checkAuth() {
    ...
    try {
      const { data: { session }, error } = await supaQuizP4Q1.auth.getSession();
...
async function fetchQuestionsFromSupabase() {
  ...
  try {
    const { data, error } = await supaQuizP4Q1
      .from('Aralin4_Quiz1')
    ...
    // Format questions for Teksto/I Know quiz
    const formattedQuestions = data.map((q) => {
      ...
      if (cleanAnswer. toLowerCase() === 'teksto') {
        cleanAnswer = 'Teksto';
      } else if (cleanAnswer.toLowerCase() === 'i know') {
        cleanAnswer = 'I Know';
      }
  ...
  function showQuestion(index) {
    ...
    // Build question HTML
    let html = `
      <legend class="sr-only">Teksto o I Know Quiz</legend>
      <div class="quiz-question-card">
        <span class="question-number">${index + 1}. </span>
        <span class="question-text">${questionData. q}</span>
        <div class="tama-mali-group">
          <label>
            <input type="radio" name="q${index + 1}" value="Teksto" ${userAnswers[index] === 'Teksto' ? 'checked' : ''} required> 
            Teksto
          </label>
          <label>
            <input type="radio" name="q${index + 1}" value="I Know" ${userAnswers[index] === 'I Know' ? 'checked' : ''}> 
            I Know
          </label>
        </div>
      </div>
...
async function submitQuiz() {
  ...
  for (let i = 0; i < questions. length; i++) {
    ...
    console.log('  Question text:', questions[i].q);
    ...
    console.log('  User answer length:', userAnswer ?  userAnswer.length : 0);
    console.log('  Correct answer length:', correctAnswer ? correctAnswer.length : 0);
    
    // Show character codes to detect hidden characters
    if (userAnswer && correctAnswer) {
      console.log('  User answer char codes:', Array.from(userAnswer).map(c => c.charCodeAt(0)));
      console.log('  Correct answer char codes:', Array.from(correctAnswer).map(c => c.charCodeAt(0)));
    }
