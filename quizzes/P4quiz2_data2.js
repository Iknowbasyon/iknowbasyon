...
const supaQuizP4Q2 = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, {
  auth: {
    persistSession: true,
...
console.log('✅ Supabase initialized for Aralin 4 Quiz 2');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function checkAuth() {
    ...
    try {
      const { data: { session }, error } = await supaQuizP4Q2.auth.getSession();
...
async function fetchQuestionsFromSupabase() {
  ...
  try {
    const { data, error } = await supaQuizP4Q2
      .from('Aralin4_Quiz2')
...
function updateTimer() {
    ...
    const timeText = `Time left: ${minutes}:${seconds}`;
    
    console.log('🕐 Updating timer:', timeText);
    console.log('Timer element:', timerDisplay);
    console.log('Timer element exists? ', !!timerDisplay);
...
function startTimer() {
    ...
    console.log('Looking for timer element...');
