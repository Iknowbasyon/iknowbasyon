...
const supaQuizP3 = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, {
  auth: {
    persistSession: true,
...
console.log('✅ Supabase initialized for Aralin 2 Quiz 3');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function checkAuth() {
    ...
    try {
      const { data: { session }, error } = await supaQuizP3.auth.getSession();
  ...
  async function fetchQuestionsFromSupabase() {
    ...
    try {
      const { data, error } = await supaQuizP3
        . from('Aralin2_Quiz3')
  ...
  if (proceedBtn) {
    proceedBtn.onclick = async function() {
      ...
      if (questions.length === 0) {
        ...
        proceedBtn.textContent = 'Start Quiz';
        return;
      }
