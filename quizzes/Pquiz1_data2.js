...
const supaQuizP1 = window.supabase. createClient(SUPABASE_URL, SUPABASE_KEY, {
  auth: {
    persistSession: true,
...
console.log('✅ Supabase initialized for Aralin 2 Quiz 1');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function checkAuth() {
    ...
    try {
      const { data: { session }, error } = await supaQuizP1.auth.getSession();
  ...
  async function fetchQuestionsFromSupabase() {
    ...
    try {
      const { data, error } = await supaQuizP1
        .from('Aralin2_Quiz1')
  ...
  // ✅ Proceed Button - Fetches Questions
  if (proceedBtn) {
    proceedBtn.onclick = async function() {
      ...
      if (headerBackBtn) {
        ...
        headerBackBtn.onclick = (e) => {
          e.preventDefault();
          alert('Hindi ka maaaring bumalik sa reading!  Kailangan mong basahin nang mabuti sa una.');
          return false;
        };
  ...
  async function submitQuiz() {
    ...
    if (currentUser) {
      ...
      const quizData = {
        ...
        quiz_name: 'Artificial Intelligence',
