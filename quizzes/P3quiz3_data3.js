...
const supaQuizP3Q3 = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, {
  auth: {
    persistSession: true,
...
console.log('✅ Supabase initialized for Aralin 3 Quiz 3');
...
document. addEventListener('DOMContentLoaded', function() {
  ...
  async function checkAuth() {
    ...
    try {
      const { data: { session }, error } = await supaQuizP3Q3.auth.getSession();
...
async function fetchQuestionsFromSupabase() {
  ...
  try {
    const { data, error } = await supaQuizP3Q3
      . from('Aralin3_Quiz3')
    ...
    // Format questions for TUMPAK/LAGAPAK quiz
    const formattedQuestions = data.map((q) => {
      ...
      if (cleanAnswer. toLowerCase() === 'tumpak') {
        cleanAnswer = 'TUMPAK';
      } else if (cleanAnswer.toLowerCase() === 'lagapak') {
        cleanAnswer = 'LAGAPAK';
      }
  ...
  function showQuestion(index) {
    ...
    // Build question HTML
    let html = `
      <legend class="sr-only">Tumpak o Lagapak Quiz</legend>
      <div class="quiz-question-card">
        <span class="question-number">${index + 1}.</span>
        <span class="question-text">${questionData.q}</span>
        <div class="tama-mali-group">
          <label>
            <input type="radio" name="q${index + 1}" value="TUMPAK" ${userAnswers[index] === 'TUMPAK' ? 'checked' : ''} required> 
            TUMPAK
          </label>
          <label>
            <input type="radio" name="q${index + 1}" value="LAGAPAK" ${userAnswers[index] === 'LAGAPAK' ? 'checked' : ''}> 
            LAGAPAK
          </label>
        </div>
      </div>
